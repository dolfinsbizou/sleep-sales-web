...
/**
 * Exception class for Astaroth
 * 
 * @package Astaroth
 */
class AstarothException extends Exception {}

/**
 * HTTP Exception class for Astaroth
 * 
 * The code must be an HTTP response code
 * 
 * @package Astaroth
 */
class AstarothHttpException extends AstarothException {}


/**
 * Astaroth Framework Main class
 *
 * @package Astaroth
 */
final class Astaroth implements ArrayAccess
{
     ...
     * Execution contexts
     * 
     * Each call to Astaroth::execute() creates a context.
     * 
     * @var array
     */
    ...
    public static function run($rootDirectory = '.', $env = null, $uri = null, $dispatch = true)
    {
        ...
        try {
            ...
            // config & environment
            try {
                self::set(self::loadConfig(self::path(self::get('astaroth.files.config'), null, false)));
            } catch (AstarothException $e) {}

            $env = $env ?: (defined('ASTAROTH_ENV') ? ASTAROTH_ENV : null);
            if ($env !== null && self::has($env)) {
                self::set(self::get($env));
            }
            
            self::fireEvent('Astaroth::Config');
            
            // sets the error reporting to all errors if debug mode is on
            if (self::get('astaroth.debug', false) == true) {
                error_reporting(E_ALL | E_STRICT);
            }

            // makes relative include dirs relative to app root
            self::set('astaroth.dirs.includes', self::path(self::get('astaroth.dirs.includes', array())));

            // registers the class autoload handler
            if (self::get('astaroth.class_autoload', true) == true) {
                if (!function_exists('spl_autoload_register')) {
                    throw new AstarothException("Missing 'spl_autoload_register()' function");
                }
                spl_autoload_register('Astaroth::needed');
            }

            // TODO Interface
            $t = self::get('plugins', array());
            if (!empty($t))
                Astaroth::path(Astaroth::get('astaroth.dirs.interface'));
                include(Astaroth::path(Astaroth::get('astaroth.dirs.interface')) . '/iPlugin.php');
            ...
            self::fireEvent('Astaroth::Bootstrap');
            if ($filename = self::path(self::get('astaroth.files.bootstrap'))) {
                include($filename);
            }

            $cancel = false;
            self::fireEvent('Astaroth::Start', array(&$cancel));
            if ($cancel) {
                self::end(true);
            }

            if (!self::has('astaroth.url_rewriting')) {
                self::set('astaroth.url_rewriting', 
            ...
            $cancel = false;
            self::fireEvent('Astaroth::Error', array($e, &$cancel));
            if (!$cancel) {
                throw $e;
            }
        }
    ...
    {
        self::fireEvent('Astaroth::Loadconfig::Before', array(&$filename));
        ...
        if (!preg_match('/.+\.(php|ini|json)$/', $filename)) {
            ...
            if (!$found) {
                throw new AstarothException("Configuration file '$filename' not found");
            }
            $filename .= ".$format";
        } else {
        ...
        if ($format === 'ini') {
            if (($data = parse_ini_file($filename, true)) === false) {
                throw new AstarothException('INI configuration malformed');
            }
            $config = self::dimensionizeArray($data, '.');
        } else if ($format === 'json') {
            if (($config = json_decode(file_get_contents($filename), true)) === null) {
                throw new AstarothException('JSON configuration malformed');
            }
        } else {
        ...
        self::fireEvent('Astaroth::Loadconfig::After', array($filename, &$config));
        return $config;
    }
    ...
    public static function dispatch($uri = null, $allowPluggableApplication = true)
    {
        try {
            $cancel = null;
            self::fireEvent('Astaroth::Dispatch::Start', array(&$uri, &$allowPluggableApplication, &$cancel));
            if ($cancel) {
                return false;
            }

            if ($uri === null) {
                $trigger = self::get('astaroth.trigger', 'action');
            ...
            if (self::get('astaroth.base_url') === null) {
                if (self::get('astaroth.url_rewriting') && (isset($_SERVER['REDIRECT_URL']) || isset($_SERVER['REDIRECT_URI']))) {
                    ...
                    if (isset($trigger) && isset($_GET[$trigger])) {
                        self::set('astaroth.base_url', substr($redirectUrl, 0, -(strlen($redirectUrl) - strpos($redirectUrl, $_GET[$trigger]))));
                    } else {
                        self::set('astaroth.base_url', $redirectUrl);
                    }
                } else {
                    // finds the base url from the script name
                    self::set('astaroth.base_url', rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\') . '/');
                }
            }
            ...
            if (strpos($request['action'], '..') !== false || substr($request['action'], 0, 1) === '_' 
                || strpos($request['action'], '/_') !== false) {
                    throw new AstarothException('Action outside of bound');
            }
            ...
            self::fireEvent('Astaroth::Dispatch::Uri', array(&$uri, &$request, &$cancel));
            if ($cancel) {
                return false;
            }
            ...
            if ($allowPluggableApplication) {
                foreach (self::$pluggableApplications as $plugin => $pluggAppConfig) {
                    ...
                    self::set('astaroth.base_action', $baseAction);
            ...
            if (!in_array($httpMethod, self::get('app.allowed_http_methods'))) {
                throw new AstarothException('HTTP method not allowed');
            }
            ...
            self::fireEvent('Astaroth::Dispatch::Before', array(&$cancel));
            if ($cancel) {
                return false;
            }
            
            $vars = array();
            if ($filename = self::path(self::get('astaroth.files.pre_dispatch'))) {
            ...
            self::fireEvent('Astaroth::Output::Before', array(&$content));
            echo $content;
            self::fireEvent('Astaroth::Output::After', array($content));
        
            self::fireEvent('Astaroth::Dispatch::After');
        
            if ($filename = self::path(self::get('astaroth.files.post_dispatch'))) {
                require($filename);
            }
            
        } catch (AstarothHttpException $e) {
            $cancel = false;
            self::fireEvent('Astaroth::Httperror', array($e, &$cancel));
            if (!$cancel) {
    ...
    /**
     * Fires the Astaroth::End event and exits the application
     *
    ...
    {
        self::fireEvent('Astaroth::End', array($success, &$writeSession));
     ...
     * For example "modules/*" will match all child actions of modules.
     * If you want to match modules and its children use "modules*".
     * 
     * Pattern is considered a regular expression if enclosed
     * between # (example: "#modules/(.*)#")
     * 
     ...
     *
     * You can also name your routes using the
     * @param string $uri
     * @param array $params Additional parameters which are not in the uri
     * @param array $routes Uses app/routes if null
     * @return array Route parameters
     * @throws AstarothException
     * @internal param $parameter (which won't be included
     * in the returned params). Named route can then be used with Astaroth::url()
     *
     */
    public static function route($uri, $params = array(), $routes = null)
    {
        ...
        self::fireEvent('Astaroth::Router::Start', array(&$uri, &$routes, &$params));
        ...
        if (self::get('app.force_uri_extension', false) && $uriExtension === false) {
            throw new AstarothException('Missing file extension');
        }
        ...
        foreach (array_reverse($routes) as $pattern => $defaults) {
            ...
            if ($pattern{0} !== '#') {
                ...
                if (substr($pattern, -2) === '/*' || self::get('astaroth.auto_uri_wildcard', false)) {
                    if (substr($pattern, -2) === '/*') {
        ...
        if (!isset($request['action'])) {
            throw new AstarothException("Missing 'action' parameter in matching route");
        }
        
        $request = array_merge($params, $request);
        self::fireEvent('Astaroth::Router::End', array($uri, &$request));
    ...
    public static function execute($action, $viewContext = true, $vars = array())
    {
        ...
        self::fireEvent('Astaroth::Execute::Start', array(&$action, &$context, &$vars));
        if ($action === false) {
        ...
        self::fireEvent('Astaroth::Execute::Before', array(&$action, &$context, &$vars));
        
        $executor = self::get('app.executor', 'Astaroth::executeFile');
        $actionExists = true;
        ...
        self::fireEvent('Astaroth::Execute::After', array($action, &$context, &$vars));
    ...
    public static function executeFile($action, $method, $vars, $context)
    {
        ...
        self::fireEvent('Astaroth::Executefile', array(&$actionFilename, &$methodActionFilename, &$context));
        if ($actionFilename === false && $methodActionFilename === false) {
            return false;
        }
        
        $astaroth = self::instance();
        if ($actionFilename !== false) {
            list($content, $vars) = $astaroth->scoped($actionFilename, $vars);
            echo $content;
        }
        if ($methodActionFilename !== false) {
            list($content, $vars) = $astaroth->scoped($methodActionFilename, $vars);
            echo $content;
        }
    ...
    {
        $dirs = self::path($dirs ?: self::get('astaroth.dirs.actions'));
     ...
     * directories configured in astaroth/dirs/views. If no file is found, an exception is thrown.
     *
    ...
    {
        $dirs = $dirs ?: self::get('astaroth.dirs.views');
        self::fireEvent('Astaroth::Render::Start', array(&$view, &$vars, &$dirs));
        if ($view === false || ($filename = self::viewFilename($view, $dirs)) === false) {
            return false;
        }
        
        self::fireEvent('Astaroth::Render::Before', array(&$view, &$vars, &$filename));
        $output = self::renderFile($filename, $vars);
        self::fireEvent('Astaroth::Render::After', array($view, &$output, $vars, $filename));
        return $output;
    }
    ...
    {
        self::fireEvent('Astaroth::Renderfile::Before', array(&$filename, &$vars));
        
        if (($callback = self::get('app.views.engine', false)) !== false) {
            if (!is_callable($callback)) {
                throw new AstarothException('The specified rendering engine callback cannot be called');
            }
        ...
        self::fireEvent('Astaroth::Renderfile::After', array($filename, &$output, $vars));
        return $output;
    }
    ...
    {
        $dirs = $dirs ?: self::get('astaroth.dirs.layouts');
        if (is_array($layout)) {
        ...
        do {
            ...
            self::fireEvent('Astaroth::Renderlayout', array(&$layout, &$content, &$vars, &$dirs));
    ...
    {
        $dirs = self::path($dirs ?: self::get('astaroth.dirs.views'));
    ...
    public static function loadHelper($helperName, $dirs = null)
    {
        ...
        self::fireEvent('Astaroth::Loadhelper::Before', array(&$helperName, &$dirs));
        ...
        if (!function_exists($functionName) && !class_exists($className)) {
            $dirs = self::path($dirs ?: self::get('astaroth.dirs.helpers'));
            if (($include = self::findFile("$helperName.php", $dirs, true)) === false) {
                throw new AstarothException("Helper '$helperName' not found");
            }
        ...
        if (function_exists($functionName)) {
            ...
            throw new AstarothException("Helper '$helperName' file found but no function or class matching the helper name");
        }
        
        self::fireEvent('Astaroth::Loadhelper::After', array($helperName, $dirs));
    }
    ...
    {
        if (method_exists('Astaroth', $helperName)) {
            return call_user_func_array(array('Astaroth', $helperName), $args);
        }
     ...
     * Redirect calls to {@see Astaroth::helper()}
     * 
     * @param string $helperName
     * @param array $args
     * @return mixed
     ...
     *
     * @see Astaroth::dimensionizeArray()
     * @param array|string $key Can be an array to set many key/value
     * @param mixed $value
     * @param bool $dimensionize Whether to use Astaroth::dimensionizeArray() on $key
     * @param array $array The array on which the operation is applied
     * @param array|bool $add Whether to add values or replace them
     * @throws AstarothException
     */
    public static function set($key, $value = null, $dimensionize = true, &$array = null, $add = false)
    {
        ...
        if (!is_array($key)) {
            throw new AstarothException("The first parameter of Astaroth::set() must be a string or an array, '" . gettype($key) . "' given");
        }
     ...
     * @param mixed $value
     * @param bool $dimensionize Whether to use Astaroth::dimensionizeArray()
     * @param array $array The array on which the operation is applied
     */
    ...
    public static function delete($key, &$array = null)
    {
        ...
        if ($parentArray === null || !self::arrayHasKey($key, $parentArray)) {
            throw new AstarothException("Key '$key' does not exists");
        }
    ...
    {
        return Astaroth::get($key);
    }

    public function offsetSet($key, $value)
    {
        return Astaroth::set($key, $value);
    }

    public function offsetExists($key)
    {
        return Astaroth::has($key);
    }

    public function offsetUnset($key)
    {
        return Astaroth::delete($key);
    }
    ...
    public static function loadCustomPlugin($plugin, &$config = array(), $options = array())
    {
        ...
        $options = array_merge(array(
            'dirs'                  => self::get('astaroth.dirs.plugins'),
            'classNameTemplate'     => '%',
        ...
        self::fireEvent('Astaroth::Plugin::Before', array(&$plugin, &$config, &$options));
        if ($plugin === false) {
            return false;
        }
        ...
        if (!class_exists($pluginClass)) {
            ...
            if (($include = self::findFile("$filename.php", $options['dirs'], true)) === false) {
                ...
                if (($include = self::findFile($filename, $options['dirs'], true)) === false) {
                    throw new AstarothException("Missing plugin '$plugin' (no file or directory matching plugin name)");
                }
                ...
                if (!($isPluggApp = file_exists($appFilename)) && !file_exists($filename)) {
                    throw new AstarothException("Missing plugin '$plugin' (no file inside the plugin's directory)");
                }
                ...
                if (!empty($ns)) {
                    self::add('astaroth.dirs.includes', array($ns => $pluginDir));
                } else {
        ...
        self::fireEvent('Astaroth::Plugin::After', array($plugin));
    ...
    {
        $plugin = ucfirst($plugin);
        $dirs = self::path(self::get('astaroth.dirs.plugins'));
    ...
    {
        $plugin = ucfirst($plugin);
        self::fireEvent('Astaroth::Registerpluggableapplication', array(&$plugin, &$route, &$config));
        if (empty($plugin)) {
            return;
        }
     ...
     *
     * @see Astaroth::registerPluggableApplication()
     * @param string $plugin Plugin's name
    ...
    public static function dispatchPluggableApplication($plugin, $uri = null, $config = array())
    {
        ...
        if (!is_dir($appDir)) {
            throw new AstarothException("To be used as an application, the plugin '$plugin' must use a directory");
        }
        
        $overrideDir = self::findFile($plugin . $rootDir, self::path(self::get('astaroth.dirs.overrides')));
        ...
        self::set('astaroth.dirs', $dirs);
        
        // rewrite files
        if ($config['overwriteFiles']) {
            $files = self::get('astaroth.files');
            $files['pre_dispatch'] = $appDir . '/pre_dispatch.php';
            $files['post_dispatch'] = $appDir . '/post_dispatch.php';
            self::set('astaroth.files', $files);
        }
        
        $cancel = false;
        self::fireEvent('Astaroth::Dispatchpluginapplication::Ready', array($plugin, &$uri, $config, &$cancel));
        if ($cancel) {
            return true;
        }
        ...
        $cancel = false;
        self::fireEvent('Astaroth::Dispatchpluginapplication::Start', array($plugin, &$uri, $config, &$cancel));
        if ($cancel) {
            return true;
        }
    ...
    public static function needed($include, $className = true)
    {
        ...
        $dirs = null;
        self::fireEvent('Astaroth::Needed', array(&$include, &$className, &$dirs));
        if ($include === null) {
            return false;
        }

        $includeDirs = (array) self::get('astaroth.dirs.includes', array());
     ...
     * @param bool $useBaseAction Whether to prepend the action with astaroth/base_action
     * @return string
     * @throws AstarothException
     */
    public static function url($action = null, $params = array(), $useIndex = true, $useBaseAction = true)
    {
        ...
        if (strlen($action) > 0 && $action{0} == '@') {
        	...
        	foreach (self::get('app.routes') as $route => $default) {
			    ...
			    if ($route{0} == '#') {
			        throw new AstarothException("Named route ('$routeName') cannot use regular expressions");
			    }
    			$action = $route;
    			break;
        	}
            if ($action === null) {
                throw new AstarothException("Missing route named '$routeName'");
            }
        }
        ...
        if (!preg_match('/^([a-z]+):\/\/.*/', $action)) {
            $action = ltrim($action, '/');
            $url = rtrim(self::get('astaroth.base_url', '.'), '/') . '/';
            if ($useBaseAction) {
                $action = ltrim(trim(self::get('astaroth.base_action', ''), '/') . '/' . $action, '/');
            }
            if (!$useIndex || self::get('astaroth.url_rewriting', false) == true) {
                $url .= $action;
            } else {
                $url .= self::get('astaroth.files.index', 'index.php');
        ...
        $args = func_get_args();
        unset($args[0]);    
        self::fireEvent('Astaroth::Url', array($action, &$url, $args));
        return $url;
    }
    
    /**
     * Returns an url exactly like Astaroth::url() but relative to the root application.
     *
     * @see Astaroth::url()
     * @param string $action
     * @param array $params
    ...
    /**
     * Returns an url exactly like Astaroth::url() but relative to a plugin
     *
     * @see Astaroth::url()
    ...
    public static function pluginUrl($plugin, $action, $params = array(), $useIndex = true)
    {
        ...
        if (!isset(self::$pluggableApplications[$plugin])) {
            throw new AstarothException("Plugin '$plugin' is not registered as a pluggable application");
        }
    ...
    {
        $template = self::get('astaroth.plugin_assets_tpl', 'app/plugins/%s/assets');
    ...
    {
        throw new AstarothHttpException($message, 404);
    }
}
...
Astaroth::reset(array(
    ...
    'app' => array(
         ...
         * @var callback */
        'router'                => 'Astaroth::route',
        
        /**
         * The callback used to execute actions
         * @var callback */
        'executor'              => 'Astaroth::executeFile',
...
Astaroth::set(array(
    ...
    'astaroth' => array(
        ...
        'dirs' => array(
            'public'             => '.',
            'interface'          => __DIR__ . '/interface',
            'plugins'            => array('Astaroth' => __DIR__ . '/plugins', 'app/plugins'),
            'actions'            => 'app/actions',
            'views'              => 'app/views',
            'layouts'            => array('app/views', 'app/layouts'),
            'helpers'            => array('Astaroth\Helpers' => __DIR__ . '/helpers', 'app/helpers'),
            'includes'           => array('Astaroth' => __DIR__ . '/plugins', 'app/includes', 'app/libs'),
